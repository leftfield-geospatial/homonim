...
def _test_vals(param_stats: list[dict]):
    """Test statistics against known values for param_file."""
    ...
    for param_band_stats, exp_param_band_stats in zip(
        param_stats, exp_param_stats_list, strict=True
    ):
        for k in exp_param_band_stats.keys():
            assert param_band_stats[k] == pytest.approx(
                exp_param_band_stats[k], abs=1e-2
            )
...
def test_api__context(param_file: Path):
    ...
    with pytest.raises(IoError):
        _ = stats.stats()
    # test ParamStats.stats usage with context entry is ok, and file is closed on
    # context exit
    with stats:
        assert len(stats.metadata) > 0
        _ = stats.stats()
...
def test_api__tables(param_file: Path):
    ...
    for band_stats in param_stats:
        for v in band_stats.values():
            assert (
                (f'{v:.3f}' in param_table)
                if isinstance(v, float)
                else (v in param_table)
            )
...
def test_cli__file_format_error(runner: CliRunner, file_rgb_100cm_float):
    """Test stats cli fails with error message when the parameter file format is
    incorrect.
    """
    cli_str = f'stats {file_rgb_100cm_float}'
    result = runner.invoke(cli, cli_str.split())
    assert result.exit_code != 0
    assert 'not a valid parameter image' in result.output
