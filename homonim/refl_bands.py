...
class MatchedPairReader(RasterPairReader):
    ...
    def _match_pair_bands(
        ...
        src_name = pathlib.Path(src_im.name).name
        ref_name = pathlib.Path(ref_im.name).name
        ...
        ref_matched = np.array([True if bi in match_bands else False for bi in ref_bands])
